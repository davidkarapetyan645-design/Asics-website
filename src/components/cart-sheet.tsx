    ...
    <AnimatePresence>
      {isOpen ? (
        <>
          ...
          <div className="fixed inset-0 z-50 sm:hidden">
            ...
            >
              ...
              <div className="flex-1 px-4 py-4">
                {items.length === 0 ? (
                  <div className="rounded-[24px] border border-dashed border-[#d7d1c3] bg-white px-5 py-10 text-center">
                    <p className="font-display text-[1.35rem] font-semibold tracking-display text-[#14171c]">
                      Cart is empty
                    </p>
                  ...
                  <div className="space-y-4">
                    {items.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-[24px] border border-[#e2dccf] bg-white p-4 shadow-[0_12px_30px_rgba(15,17,21,0.06)]"
                      >
                        <div className="flex gap-3">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                            alt={item.name}
                            className="h-20 w-20 rounded-[18px] border border-[#ece6d9] bg-[#f5f2ea] object-contain p-2"
                            src={item.image}
                          />
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <p className="font-display text-[1.05rem] font-semibold tracking-display text-[#14171c]">
                                  {item.name}
                                </p>
                              ...
                              <button
                                className="text-sm font-medium text-[#8a4b4b]"
                            ...
                            <div className="mt-4 flex items-center justify-between gap-3">
                              ...
                              <p className="font-display text-[1.1rem] font-semibold tracking-display text-[#14171c]">
              ...
              <div className="sticky bottom-0 border-t border-[#e6e0d4] bg-[#fbfaf6]/96 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-4 backdrop-blur-sm">
                <div className="rounded-[24px] border border-[#ddd7ca] bg-white p-4">
                  ...
                  <button
                    className="mt-5 w-full rounded-full bg-[#16191f] px-5 py-4 text-sm font-semibold uppercase tracking-[0.18em] text-white disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <motion.aside
            className="fixed right-0 top-0 z-50 hidden h-full w-full max-w-[420px] flex-col border-l border-[#ddd7ca] bg-[#fbfaf6] text-[#111318] shadow-[0_24px_80px_rgba(15,17,21,0.18)] sm:flex"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
